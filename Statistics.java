        ...
        public void revenuePatient(HashMap<Integer, Appointment> appointments, HashMap<Integer, Exam> exams, HashMap<Integer, Patient> patients){
                ...
                for ( Patient patient : patients.values()){  //we loop through all the patients
                        ...
                        for(Appointment appointment : appointments.values()){  //we loop through all the appointments to check if the patient has any appointments
                                if (appointment.getPatientID() == patient.getCode() && !appointment.getDeleted()){  //if the patientID of the appointment matches the code of the patient and the appointment is not deleted we get the cost of the exam and add it to the sum of the patient
                                        for (Exam exam:exams.values()){  
                                                if(appointment.getExamID() == exam.getCode()){  //we check if the examID of the appointment matches the code of the exam, if it does we get the cost of the exam to print it later and add it to the sum of the patient
